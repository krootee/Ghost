 ...
 * Mimics the 'map' function in Arduino
 */
 ...
 * TODO - does it make sense that the value also controls the direction?
 * Perhaps direction should be a separate thing?
 */
 ...
 * Motor is first stopped, then a slight delay, and then direction is changed.
 * The idea is that changing direction quickly shouldn't break the motor.
 */
...
/*
 * Call calibration on the motor. Useful for calibrating ESC (Electronic Speed Control).
 * Maximum throttle for some time, then minimum throttle for some time, and lastly center for some time.
 */
void Motor::calibrate()
{
	...
	this->SetDutyCycle(center); * Motor.cpp
...
/*
 * Call calibration on the motor.
 * Maximum throttle for some time, then minimum throttle for some time, and lastly center for some time.
 */
void Motor::calibrate()
{
	//TODO: pass in the delay time as a parameter
