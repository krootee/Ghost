EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:NorBot
LIBS:switches
LIBS:GhostESP32-cache
EELAYER 26 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 2
Title "Ghost ESP32"
Date "2017-03-10"
Rev "2"
Comp "Frode Lillerud"
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text Notes 4850 800  0    98   ~ 20
Ghost ESP32 shield
Text Notes 4150 1200 0    60   ~ 0
A shield based around the ESP-WROOM-32\nIt uses a stackable header from Hirose to connect to other boards.\nBoard is powered by 5.0V either through MicroUSB, or via 40-pin connector.
$Comp
L USB_OTG-RESCUE-GhostESP32 P1
U 1 1 5893902B
P 8900 1200
F 0 "P1" H 9225 1075 50  0000 C CNN
F 1 "USB_OTG" H 8900 1400 50  0000 C CNN
F 2 "Connectors:USB_Micro-B" V 8850 1100 50  0001 C CNN
F 3 "" V 8850 1100 50  0000 C CNN
	1    8900 1200
	0    -1   1    0   
$EndComp
$Comp
L LED D3
U 1 1 58939127
P 2350 4500
F 0 "D3" H 2350 4600 50  0000 C CNN
F 1 "RXLED" H 2350 4400 50  0000 C CNN
F 2 "LEDs:LED_0603" H 2350 4500 50  0001 C CNN
F 3 "" H 2350 4500 50  0000 C CNN
	1    2350 4500
	-1   0    0    1   
$EndComp
$Comp
L LED D4
U 1 1 589391B9
P 2400 3300
F 0 "D4" H 2400 3400 50  0000 C CNN
F 1 "TXLED" H 2400 3200 50  0000 C CNN
F 2 "LEDs:LED_0603" H 2400 3300 50  0001 C CNN
F 3 "" H 2400 3300 50  0000 C CNN
	1    2400 3300
	-1   0    0    1   
$EndComp
$Comp
L CP2102 U3
U 1 1 589391DD
P 5350 5850
F 0 "U3" H 5900 5250 60  0000 C CNN
F 1 "CP2102" H 5350 5850 60  0000 C CNN
F 2 "Housings_DFN_QFN:QFN-28-1EP_5x5mm_Pitch0.5mm" H 5400 5850 60  0001 C CNN
F 3 "" H 5400 5850 60  0001 C CNN
	1    5350 5850
	1    0    0    -1  
$EndComp
Text Notes 9800 2250 0    60   ~ 0
Topside connector, 40 pin\nHirose FX8C-40P-SV\nFarnell no: 249-8677
Text GLabel 7650 2000 2    39   Input ~ 0
IO23
$Comp
L GND #PWR01
U 1 1 5893D6AF
P 5250 3550
F 0 "#PWR01" H 5250 3300 50  0001 C CNN
F 1 "GND" H 5250 3400 50  0000 C CNN
F 2 "" H 5250 3550 50  0000 C CNN
F 3 "" H 5250 3550 50  0000 C CNN
	1    5250 3550
	1    0    0    -1  
$EndComp
Text GLabel 7650 2100 2    39   Input ~ 0
IO22
Text GLabel 7650 2200 2    39   Input ~ 0
RXD
Text GLabel 7650 2300 2    39   Input ~ 0
TXD
Text GLabel 7650 2400 2    39   Input ~ 0
IO21
Text GLabel 7650 2600 2    39   Input ~ 0
IO19
Text GLabel 7650 2700 2    39   Input ~ 0
IO18
Text GLabel 7650 2800 2    39   Input ~ 0
IO5
Text GLabel 7650 2900 2    39   Input ~ 0
IO17
Text GLabel 7650 3000 2    39   Input ~ 0
IO16
Text GLabel 7650 3100 2    39   Input ~ 0
IO4
$Comp
L GND #PWR02
U 1 1 5893F039
P 7500 3500
F 0 "#PWR02" H 7500 3250 50  0001 C CNN
F 1 "GND" H 7500 3350 50  0000 C CNN
F 2 "" H 7500 3500 50  0000 C CNN
F 3 "" H 7500 3500 50  0000 C CNN
	1    7500 3500
	1    0    0    -1  
$EndComp
Text GLabel 5050 2300 0    39   Input ~ 0
IO12
Text GLabel 5050 2400 0    39   Input ~ 0
IO14
Text GLabel 5050 2500 0    39   Input ~ 0
IO27
Text GLabel 5050 2600 0    39   Input ~ 0
IO26
Text GLabel 5050 2700 0    39   Input ~ 0
IO25
Text GLabel 5050 2800 0    39   Input ~ 0
IO33
Text GLabel 5050 2900 0    39   Input ~ 0
IO32
Text GLabel 5050 3000 0    39   Input ~ 0
IO35
Text GLabel 5050 3100 0    39   Input ~ 0
IO34
Text GLabel 5050 3200 0    39   Input ~ 0
SENSOR_VN
Text GLabel 5050 3300 0    39   Input ~ 0
SENSOR_VP
Text GLabel 6050 4050 3    39   Input ~ 0
IO13
Text GLabel 6750 4050 3    39   Input ~ 0
IO15
$Comp
L GND #PWR03
U 1 1 5894216D
P 5950 4200
F 0 "#PWR03" H 5950 3950 50  0001 C CNN
F 1 "GND" H 5950 4050 50  0000 C CNN
F 2 "" H 5950 4200 50  0000 C CNN
F 3 "" H 5950 4200 50  0000 C CNN
	1    5950 4200
	1    0    0    -1  
$EndComp
$Comp
L C C6
U 1 1 58945389
P 4250 2300
F 0 "C6" H 4275 2400 50  0000 L CNN
F 1 "1uF" H 4275 2200 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 4288 2150 50  0001 C CNN
F 3 "" H 4250 2300 50  0000 C CNN
	1    4250 2300
	1    0    0    -1  
$EndComp
$Comp
L C C4
U 1 1 589455FB
P 3900 2300
F 0 "C4" H 3925 2400 50  0000 L CNN
F 1 "100uF" H 3925 2200 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 3938 2150 50  0001 C CNN
F 3 "" H 3900 2300 50  0000 C CNN
	1    3900 2300
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR04
U 1 1 5894566B
P 3900 2600
F 0 "#PWR04" H 3900 2350 50  0001 C CNN
F 1 "GND" H 3900 2450 50  0000 C CNN
F 2 "" H 3900 2600 50  0000 C CNN
F 3 "" H 3900 2600 50  0000 C CNN
	1    3900 2600
	1    0    0    -1  
$EndComp
$Comp
L +3V3 #PWR05
U 1 1 58945BE9
P 3900 1900
F 0 "#PWR05" H 3900 1750 50  0001 C CNN
F 1 "+3V3" H 3900 2040 50  0000 C CNN
F 2 "" H 3900 1900 50  0000 C CNN
F 3 "" H 3900 1900 50  0000 C CNN
	1    3900 1900
	1    0    0    -1  
$EndComp
Text GLabel 5050 2150 0    39   Input ~ 0
EN
$Comp
L R R11
U 1 1 589470F2
P 8100 5500
F 0 "R11" V 8180 5500 50  0000 C CNN
F 1 "12K" V 8100 5500 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 8030 5500 50  0001 C CNN
F 3 "" H 8100 5500 50  0000 C CNN
	1    8100 5500
	1    0    0    -1  
$EndComp
$Comp
L C C10
U 1 1 58947803
P 8100 5950
F 0 "C10" H 8125 6050 50  0000 L CNN
F 1 "1nF" H 8125 5850 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 8138 5800 50  0001 C CNN
F 3 "" H 8100 5950 50  0000 C CNN
	1    8100 5950
	1    0    0    -1  
$EndComp
$Comp
L +3V3 #PWR06
U 1 1 5894816C
P 8100 5300
F 0 "#PWR06" H 8100 5150 50  0001 C CNN
F 1 "+3V3" H 8100 5440 50  0000 C CNN
F 2 "" H 8100 5300 50  0000 C CNN
F 3 "" H 8100 5300 50  0000 C CNN
	1    8100 5300
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR07
U 1 1 589485A6
P 8100 6150
F 0 "#PWR07" H 8100 5900 50  0001 C CNN
F 1 "GND" H 8100 6000 50  0000 C CNN
F 2 "" H 8100 6150 50  0000 C CNN
F 3 "" H 8100 6150 50  0000 C CNN
	1    8100 6150
	1    0    0    -1  
$EndComp
$Comp
L R R10
U 1 1 5894B448
P 7800 5750
F 0 "R10" V 7880 5750 50  0000 C CNN
F 1 "470" V 7800 5750 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 7730 5750 50  0001 C CNN
F 3 "" H 7800 5750 50  0000 C CNN
	1    7800 5750
	0    1    1    0   
$EndComp
$Comp
L GND #PWR08
U 1 1 5894B44F
P 6850 6100
F 0 "#PWR08" H 6850 5850 50  0001 C CNN
F 1 "GND" H 6850 5950 50  0000 C CNN
F 2 "" H 6850 6100 50  0000 C CNN
F 3 "" H 6850 6100 50  0000 C CNN
	1    6850 6100
	1    0    0    -1  
$EndComp
$Comp
L C C9
U 1 1 5894B455
P 7250 5900
F 0 "C9" H 7275 6000 50  0000 L CNN
F 1 "1nF" H 7275 5800 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 7288 5750 50  0001 C CNN
F 3 "" H 7250 5900 50  0000 C CNN
	1    7250 5900
	0    1    1    0   
$EndComp
Text GLabel 8250 5750 2    39   Input ~ 0
EN
$Comp
L GND #PWR09
U 1 1 5894D737
P 5050 5000
F 0 "#PWR09" H 5050 4750 50  0001 C CNN
F 1 "GND" H 5050 4850 50  0000 C CNN
F 2 "" H 5050 5000 50  0000 C CNN
F 3 "" H 5050 5000 50  0000 C CNN
	1    5050 5000
	-1   0    0    1   
$EndComp
Text GLabel 5150 5000 1    39   Input ~ 0
DTR
$Comp
L R R7
U 1 1 5894DAE4
P 5350 4750
F 0 "R7" V 5430 4750 50  0000 C CNN
F 1 "470" V 5350 4750 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 5280 4750 50  0001 C CNN
F 3 "" H 5350 4750 50  0000 C CNN
	1    5350 4750
	-1   0    0    1   
$EndComp
Text GLabel 5350 4500 1    39   Input ~ 0
TXD
Text GLabel 5450 5000 1    39   Input ~ 0
RXD
Text GLabel 5550 5000 1    39   Input ~ 0
RTS
$Comp
L GND #PWR010
U 1 1 5894F1A1
P 4350 5750
F 0 "#PWR010" H 4350 5500 50  0001 C CNN
F 1 "GND" H 4350 5600 50  0000 C CNN
F 2 "" H 4350 5750 50  0000 C CNN
F 3 "" H 4350 5750 50  0000 C CNN
	1    4350 5750
	0    1    1    0   
$EndComp
Text GLabel 4450 5850 0    39   Input ~ 0
USB_DP
Text GLabel 4450 5950 0    39   Input ~ 0
USB_DN
$Comp
L C C5
U 1 1 5895017F
P 3950 6300
F 0 "C5" H 3975 6400 50  0000 L CNN
F 1 "0.1uF" H 3975 6200 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 3988 6150 50  0001 C CNN
F 3 "" H 3950 6300 50  0000 C CNN
	1    3950 6300
	1    0    0    -1  
$EndComp
$Comp
L C C3
U 1 1 58950185
P 3600 6300
F 0 "C3" H 3625 6400 50  0000 L CNN
F 1 "10uF" H 3625 6200 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 3638 6150 50  0001 C CNN
F 3 "" H 3600 6300 50  0000 C CNN
	1    3600 6300
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR011
U 1 1 5895018B
P 3600 6600
F 0 "#PWR011" H 3600 6350 50  0001 C CNN
F 1 "GND" H 3600 6450 50  0000 C CNN
F 2 "" H 3600 6600 50  0000 C CNN
F 3 "" H 3600 6600 50  0000 C CNN
	1    3600 6600
	1    0    0    -1  
$EndComp
$Comp
L +3V3 #PWR012
U 1 1 58950191
P 3600 5900
F 0 "#PWR012" H 3600 5750 50  0001 C CNN
F 1 "+3V3" H 3600 6040 50  0000 C CNN
F 2 "" H 3600 5900 50  0000 C CNN
F 3 "" H 3600 5900 50  0000 C CNN
	1    3600 5900
	1    0    0    -1  
$EndComp
Wire Wire Line
	7650 2000 7300 2000
Wire Wire Line
	5400 3450 5250 3450
Wire Wire Line
	5250 3450 5250 3550
Wire Wire Line
	7650 2100 7300 2100
Wire Wire Line
	7650 2200 7300 2200
Wire Wire Line
	7650 2300 7300 2300
Wire Wire Line
	7650 2400 7300 2400
Wire Wire Line
	7650 2600 7300 2600
Wire Wire Line
	7650 2700 7300 2700
Wire Wire Line
	7650 2800 7300 2800
Wire Wire Line
	7650 2900 7300 2900
Wire Wire Line
	7650 3000 7300 3000
Wire Wire Line
	7650 3100 7300 3100
Wire Wire Line
	7500 3300 7500 3500
Wire Wire Line
	7500 3400 7300 3400
Wire Wire Line
	7300 3300 7500 3300
Connection ~ 7500 3400
Wire Wire Line
	5400 3300 5050 3300
Wire Wire Line
	5050 3200 5400 3200
Wire Wire Line
	5400 3100 5050 3100
Wire Wire Line
	5050 3000 5400 3000
Wire Wire Line
	5400 2900 5050 2900
Wire Wire Line
	5050 2800 5400 2800
Wire Wire Line
	5400 2700 5050 2700
Wire Wire Line
	5050 2600 5400 2600
Wire Wire Line
	5400 2500 5050 2500
Wire Wire Line
	5050 2400 5400 2400
Wire Wire Line
	5400 2300 5050 2300
Wire Wire Line
	5950 4200 5950 3850
Wire Wire Line
	6050 3850 6050 4050
Wire Wire Line
	6750 4050 6750 3850
Wire Wire Line
	3900 2050 5400 2050
Wire Wire Line
	3900 1900 3900 2150
Connection ~ 3900 2050
Wire Wire Line
	4250 2150 4250 2050
Connection ~ 4250 2050
Wire Wire Line
	4250 2450 4250 2550
Wire Wire Line
	4250 2550 3900 2550
Wire Wire Line
	3900 2450 3900 2600
Connection ~ 3900 2550
Wire Wire Line
	5400 2150 5050 2150
Wire Wire Line
	8100 5650 8100 5800
Connection ~ 8100 5750
Wire Wire Line
	8100 6150 8100 6100
Wire Wire Line
	8100 5350 8100 5300
Wire Wire Line
	7050 5650 6850 5650
Wire Wire Line
	6850 5650 6850 6100
Wire Wire Line
	7100 5900 6850 5900
Connection ~ 6850 5900
Wire Wire Line
	5050 5150 5050 5000
Wire Wire Line
	5150 5000 5150 5150
Wire Wire Line
	5350 5150 5350 4900
Wire Wire Line
	5350 4600 5350 4500
Wire Wire Line
	5450 5000 5450 5150
Wire Wire Line
	5550 5150 5550 5000
Wire Wire Line
	4350 5750 4600 5750
Wire Wire Line
	4600 5850 4450 5850
Wire Wire Line
	4450 5950 4600 5950
Wire Wire Line
	3600 5900 3600 6150
Wire Wire Line
	3950 6150 3950 6050
Wire Wire Line
	3950 6450 3950 6550
Wire Wire Line
	3950 6550 3600 6550
Wire Wire Line
	3600 6450 3600 6600
Connection ~ 3600 6550
Wire Wire Line
	3600 6050 4600 6050
Connection ~ 3600 6050
Connection ~ 3950 6050
Wire Wire Line
	4600 6150 4400 6150
Wire Wire Line
	4400 6150 4400 6050
Connection ~ 4400 6050
$Comp
L C C7
U 1 1 58951A89
P 4500 7050
F 0 "C7" H 4525 7150 50  0000 L CNN
F 1 "10uF" H 4525 6950 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 4538 6900 50  0001 C CNN
F 3 "" H 4500 7050 50  0000 C CNN
	1    4500 7050
	1    0    0    -1  
$EndComp
$Comp
L C C8
U 1 1 58951B2C
P 4900 7050
F 0 "C8" H 4925 7150 50  0000 L CNN
F 1 "0.1uF" H 4925 6950 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 4938 6900 50  0001 C CNN
F 3 "" H 4900 7050 50  0000 C CNN
	1    4900 7050
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR013
U 1 1 58951D50
P 4500 7400
F 0 "#PWR013" H 4500 7150 50  0001 C CNN
F 1 "GND" H 4500 7250 50  0000 C CNN
F 2 "" H 4500 7400 50  0000 C CNN
F 3 "" H 4500 7400 50  0000 C CNN
	1    4500 7400
	1    0    0    -1  
$EndComp
Wire Wire Line
	5100 6800 5100 6600
Wire Wire Line
	4500 6800 5100 6800
Wire Wire Line
	4500 6650 4500 6900
Connection ~ 4500 6800
Wire Wire Line
	4900 6800 4900 6900
Connection ~ 4900 6800
Wire Wire Line
	4900 7200 4900 7300
Wire Wire Line
	4900 7300 4500 7300
Wire Wire Line
	4500 7200 4500 7400
Connection ~ 4500 7300
NoConn ~ 5200 6600
NoConn ~ 5500 6600
Text GLabel 750  6450 0    39   Input ~ 0
DTR
Text GLabel 750  7150 0    39   Input ~ 0
RTS
$Comp
L R R3
U 1 1 58956C1E
P 1050 6450
F 0 "R3" V 1130 6450 50  0000 C CNN
F 1 "12K" V 1050 6450 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 980 6450 50  0001 C CNN
F 3 "" H 1050 6450 50  0000 C CNN
	1    1050 6450
	0    1    1    0   
$EndComp
$Comp
L R R4
U 1 1 58956CD3
P 1050 7150
F 0 "R4" V 1130 7150 50  0000 C CNN
F 1 "12K" V 1050 7150 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 980 7150 50  0001 C CNN
F 3 "" H 1050 7150 50  0000 C CNN
	1    1050 7150
	0    1    1    0   
$EndComp
Text GLabel 1850 7450 2    39   Input ~ 0
IO0
Text GLabel 1850 6150 2    39   Input ~ 0
EN
Wire Wire Line
	1850 7450 1650 7450
Wire Wire Line
	1650 7450 1650 7350
Wire Wire Line
	1350 7150 1200 7150
Wire Wire Line
	750  7150 900  7150
Wire Wire Line
	1650 6650 1650 6700
Wire Wire Line
	1650 6700 850  6700
Wire Wire Line
	850  6700 850  7150
Connection ~ 850  7150
Wire Wire Line
	1650 6950 1650 6850
Wire Wire Line
	1650 6850 800  6850
Wire Wire Line
	800  6850 800  6450
Wire Wire Line
	750  6450 900  6450
Connection ~ 800  6450
Wire Wire Line
	1200 6450 1350 6450
Wire Wire Line
	1650 6250 1650 6150
Wire Wire Line
	1650 6150 1850 6150
$Comp
L GND #PWR014
U 1 1 5895DDF4
P 9350 1650
F 0 "#PWR014" H 9350 1400 50  0001 C CNN
F 1 "GND" H 9350 1500 50  0000 C CNN
F 2 "" H 9350 1650 50  0000 C CNN
F 3 "" H 9350 1650 50  0000 C CNN
	1    9350 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	9350 1650 8800 1650
Wire Wire Line
	8800 1650 8800 1600
Wire Wire Line
	9200 1400 9350 1400
Wire Wire Line
	9350 1400 9350 1650
$Comp
L D_Schottky D5
U 1 1 5895E8E3
P 9750 1000
F 0 "D5" H 9750 1100 50  0000 C CNN
F 1 "SSA33L Schottky" H 9750 900 50  0000 C CNN
F 2 "Diodes_SMD:D_SMA_Standard" H 9750 1000 50  0001 C CNN
F 3 "" H 9750 1000 50  0000 C CNN
	1    9750 1000
	-1   0    0    1   
$EndComp
Wire Wire Line
	9600 1000 9200 1000
Wire Wire Line
	9900 1000 10250 1000
Text GLabel 9350 1200 2    39   Input ~ 0
USB_DP
Text GLabel 9350 1100 2    39   Input ~ 0
USB_DN
NoConn ~ 9200 1300
Wire Wire Line
	9350 1200 9200 1200
Wire Wire Line
	9200 1100 9350 1100
$Comp
L LED D1
U 1 1 58962246
P 750 1550
F 0 "D1" H 750 1650 50  0000 C CNN
F 1 "RED LED" H 750 1450 50  0000 C CNN
F 2 "LEDs:LED_0603" H 750 1550 50  0001 C CNN
F 3 "" H 750 1550 50  0000 C CNN
	1    750  1550
	0    -1   -1   0   
$EndComp
$Comp
L R R1
U 1 1 58962B36
P 750 1150
F 0 "R1" V 830 1150 50  0000 C CNN
F 1 "2K" V 750 1150 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 680 1150 50  0001 C CNN
F 3 "" H 750 1150 50  0000 C CNN
	1    750  1150
	1    0    0    -1  
$EndComp
$Comp
L C C1
U 1 1 58962BA1
P 1150 1250
F 0 "C1" H 1175 1350 50  0000 L CNN
F 1 "10uF" H 1175 1150 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 1188 1100 50  0001 C CNN
F 3 "" H 1150 1250 50  0000 C CNN
	1    1150 1250
	1    0    0    -1  
$EndComp
Wire Wire Line
	750  950  1600 950 
$Comp
L GND #PWR015
U 1 1 58963D1C
P 950 1900
F 0 "#PWR015" H 950 1650 50  0001 C CNN
F 1 "GND" H 950 1750 50  0000 C CNN
F 2 "" H 950 1900 50  0000 C CNN
F 3 "" H 950 1900 50  0000 C CNN
	1    950  1900
	1    0    0    -1  
$EndComp
Wire Wire Line
	750  1000 750  950 
Wire Wire Line
	750  1400 750  1300
Wire Wire Line
	1150 750  1150 1100
Connection ~ 1150 950 
Wire Wire Line
	750  1700 750  1900
Wire Wire Line
	750  1900 1150 1900
Wire Wire Line
	1150 1900 1150 1400
Connection ~ 950  1900
$Comp
L GND #PWR016
U 1 1 589652BD
P 1900 1400
F 0 "#PWR016" H 1900 1150 50  0001 C CNN
F 1 "GND" H 1900 1250 50  0000 C CNN
F 2 "" H 1900 1400 50  0000 C CNN
F 3 "" H 1900 1400 50  0000 C CNN
	1    1900 1400
	1    0    0    -1  
$EndComp
Wire Wire Line
	1900 1250 1900 1400
$Comp
L +3V3 #PWR017
U 1 1 58965D74
P 2800 950
F 0 "#PWR017" H 2800 800 50  0001 C CNN
F 1 "+3V3" H 2800 1090 50  0000 C CNN
F 2 "" H 2800 950 50  0000 C CNN
F 3 "" H 2800 950 50  0000 C CNN
	1    2800 950 
	1    0    0    -1  
$EndComp
$Comp
L C C2
U 1 1 58966607
P 2600 1200
F 0 "C2" H 2625 1300 50  0000 L CNN
F 1 "10uF" H 2625 1100 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603" H 2638 1050 50  0001 C CNN
F 3 "" H 2600 1200 50  0000 C CNN
	1    2600 1200
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR018
U 1 1 58966714
P 2600 1450
F 0 "#PWR018" H 2600 1200 50  0001 C CNN
F 1 "GND" H 2600 1300 50  0000 C CNN
F 2 "" H 2600 1450 50  0000 C CNN
F 3 "" H 2600 1450 50  0000 C CNN
	1    2600 1450
	1    0    0    -1  
$EndComp
Wire Wire Line
	2200 950  2800 950 
Wire Wire Line
	2600 1050 2600 950 
Connection ~ 2600 950 
Wire Wire Line
	2600 1450 2600 1350
$Comp
L LM358 U1
U 1 1 58967B45
P 1400 3300
F 0 "U1" H 1400 3500 50  0000 L CNN
F 1 "LM358" H 1400 3100 50  0000 L CNN
F 2 "Housings_SOIC:SOIC-8_3.9x4.9mm_Pitch1.27mm" H 1400 3300 50  0001 C CNN
F 3 "" H 1400 3300 50  0000 C CNN
	1    1400 3300
	1    0    0    -1  
$EndComp
$Comp
L R R5
U 1 1 58968A55
P 2000 3300
F 0 "R5" V 2080 3300 50  0000 C CNN
F 1 "1k" V 2000 3300 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 1930 3300 50  0001 C CNN
F 3 "" H 2000 3300 50  0000 C CNN
	1    2000 3300
	0    1    1    0   
$EndComp
$Comp
L R R6
U 1 1 58968B3C
P 2000 4500
F 0 "R6" V 2080 4500 50  0000 C CNN
F 1 "1k" V 2000 4500 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 1930 4500 50  0001 C CNN
F 3 "" H 2000 4500 50  0000 C CNN
	1    2000 4500
	0    1    1    0   
$EndComp
$Comp
L LM358 U1
U 2 1 58968C0C
P 1400 4500
F 0 "U1" H 1400 4700 50  0000 L CNN
F 1 "LM358" H 1400 4300 50  0000 L CNN
F 2 "Housings_SOIC:SOIC-8_3.9x4.9mm_Pitch1.27mm" H 1400 4500 50  0001 C CNN
F 3 "" H 1400 4500 50  0000 C CNN
	2    1400 4500
	1    0    0    -1  
$EndComp
Text GLabel 850  4400 0    39   Input ~ 0
RXD
Text GLabel 850  3200 0    39   Input ~ 0
TXD
Wire Wire Line
	1100 4400 850  4400
Wire Wire Line
	1100 3200 850  3200
$Comp
L +3V3 #PWR019
U 1 1 58969FC9
P 1300 2900
F 0 "#PWR019" H 1300 2750 50  0001 C CNN
F 1 "+3V3" H 1300 3040 50  0000 C CNN
F 2 "" H 1300 2900 50  0000 C CNN
F 3 "" H 1300 2900 50  0000 C CNN
	1    1300 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	1300 3000 1300 2900
$Comp
L GND #PWR020
U 1 1 5896BA43
P 1300 3700
F 0 "#PWR020" H 1300 3450 50  0001 C CNN
F 1 "GND" H 1300 3550 50  0000 C CNN
F 2 "" H 1300 3700 50  0000 C CNN
F 3 "" H 1300 3700 50  0000 C CNN
	1    1300 3700
	1    0    0    -1  
$EndComp
Wire Wire Line
	1300 3600 1300 3700
$Comp
L +3V3 #PWR021
U 1 1 5896C6A4
P 1300 4150
F 0 "#PWR021" H 1300 4000 50  0001 C CNN
F 1 "+3V3" H 1300 4290 50  0000 C CNN
F 2 "" H 1300 4150 50  0000 C CNN
F 3 "" H 1300 4150 50  0000 C CNN
	1    1300 4150
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR022
U 1 1 5896C7C2
P 1300 4900
F 0 "#PWR022" H 1300 4650 50  0001 C CNN
F 1 "GND" H 1300 4750 50  0000 C CNN
F 2 "" H 1300 4900 50  0000 C CNN
F 3 "" H 1300 4900 50  0000 C CNN
	1    1300 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	1300 4200 1300 4150
Wire Wire Line
	1300 4800 1300 4900
Wire Wire Line
	1700 4500 1850 4500
Wire Wire Line
	1700 3300 1850 3300
Wire Wire Line
	1100 3400 900  3400
Wire Wire Line
	900  3400 900  3600
Wire Wire Line
	900  3600 1800 3600
Wire Wire Line
	1800 3600 1800 3300
Connection ~ 1800 3300
Wire Wire Line
	1800 4500 1800 5200
Wire Wire Line
	1000 5200 1000 4600
Wire Wire Line
	1000 4600 1100 4600
Connection ~ 1800 4500
Wire Wire Line
	1300 4850 2550 4850
Connection ~ 1300 4850
Wire Wire Line
	2200 4500 2150 4500
Wire Wire Line
	2150 3300 2250 3300
Wire Wire Line
	1300 3650 2600 3650
Wire Wire Line
	2600 3650 2600 3300
Wire Wire Line
	2600 3300 2550 3300
Connection ~ 1300 3650
$Comp
L +5V #PWR023
U 1 1 5898B1F6
P 1150 750
F 0 "#PWR023" H 1150 600 50  0001 C CNN
F 1 "+5V" H 1150 890 50  0000 C CNN
F 2 "" H 1150 750 50  0000 C CNN
F 3 "" H 1150 750 50  0000 C CNN
	1    1150 750 
	1    0    0    -1  
$EndComp
$Comp
L +5V #PWR024
U 1 1 5898B7AD
P 4500 6650
F 0 "#PWR024" H 4500 6500 50  0001 C CNN
F 1 "+5V" H 4500 6790 50  0000 C CNN
F 2 "" H 4500 6650 50  0000 C CNN
F 3 "" H 4500 6650 50  0000 C CNN
	1    4500 6650
	1    0    0    -1  
$EndComp
$Comp
L +5V #PWR025
U 1 1 5898C3A8
P 10250 950
F 0 "#PWR025" H 10250 800 50  0001 C CNN
F 1 "+5V" H 10250 1090 50  0000 C CNN
F 2 "" H 10250 950 50  0000 C CNN
F 3 "" H 10250 950 50  0000 C CNN
	1    10250 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	10250 1000 10250 950 
Wire Wire Line
	2500 4500 2550 4500
Wire Wire Line
	2550 4500 2550 4850
Wire Wire Line
	1800 5200 1000 5200
$Comp
L PWR_FLAG #FLG026
U 1 1 5899E941
P 10250 1000
F 0 "#FLG026" H 10250 1095 50  0001 C CNN
F 1 "PWR_FLAG" H 10250 1180 50  0000 C CNN
F 2 "" H 10250 1000 50  0000 C CNN
F 3 "" H 10250 1000 50  0000 C CNN
	1    10250 1000
	-1   0    0    1   
$EndComp
NoConn ~ 4600 5550
NoConn ~ 4600 5650
NoConn ~ 5250 5150
Text Notes 1850 6900 0    60   ~ 0
MMBT2222A NPN transistors\nFarnell no: 177-3626
$Comp
L LED D2
U 1 1 589FED00
P 1500 5800
F 0 "D2" H 1500 5900 50  0000 C CNN
F 1 "LED" H 1500 5700 50  0000 C CNN
F 2 "LEDs:LED_0603" H 1500 5800 50  0001 C CNN
F 3 "" H 1500 5800 50  0000 C CNN
	1    1500 5800
	-1   0    0    1   
$EndComp
$Comp
L R R2
U 1 1 589FF420
P 1050 5800
F 0 "R2" V 1130 5800 50  0000 C CNN
F 1 "390" V 1050 5800 50  0000 C CNN
F 2 "Resistors_SMD:R_0603" V 980 5800 50  0001 C CNN
F 3 "" H 1050 5800 50  0000 C CNN
	1    1050 5800
	0    1    1    0   
$EndComp
$Comp
L GND #PWR027
U 1 1 58A0017B
P 1900 5800
F 0 "#PWR027" H 1900 5550 50  0001 C CNN
F 1 "GND" H 1900 5650 50  0000 C CNN
F 2 "" H 1900 5800 50  0000 C CNN
F 3 "" H 1900 5800 50  0000 C CNN
	1    1900 5800
	1    0    0    -1  
$EndComp
Wire Wire Line
	750  5800 900  5800
Wire Wire Line
	1200 5800 1350 5800
Wire Wire Line
	1650 5800 1900 5800
Text Notes 850  5650 0    60   ~ 0
Generic LED (Red)
Text GLabel 750  5800 0    39   Input ~ 0
IO23
Text Notes 8600 800  0    60   ~ 0
MicroUSB\nFarnell no: 229-3836
$Comp
L Q_NPN_BEC Q1
U 1 1 58B18998
P 1550 6450
F 0 "Q1" H 1741 6496 50  0000 L CNN
F 1 "Q_NPN_BEC" H 1741 6405 50  0000 L CNN
F 2 "TO_SOT_Packages_SMD:SOT-23" H -600 150 50  0001 C CNN
F 3 "" H -800 50  50  0001 C CNN
	1    1550 6450
	1    0    0    -1  
$EndComp
$Comp
L Q_NPN_BEC Q2
U 1 1 58B18CD1
P 1550 7150
F 0 "Q2" H 1741 7104 50  0000 L CNN
F 1 "Q_NPN_BEC" H 1741 7195 50  0000 L CNN
F 2 "TO_SOT_Packages_SMD:SOT-23" H -600 850 50  0001 C CNN
F 3 "" H -800 750 50  0001 C CNN
	1    1550 7150
	1    0    0    1   
$EndComp
Text Notes 1550 1950 0    60   ~ 0
NCP1117ST33T3G\nFarnel no: 165-2366\n1.07V dropout, 1A out
Text Notes 3900 1650 0    60   ~ 0
Note, GPIO 34 - 39 are INPUT mode ONLY.\nNote, GPIO 6-11 are used by SPI-flash, and cannot be used for other purposes.\nNote, GPIO0, GPIO2, GPIO5, GPIO12 and GPIO15 are strapping pins, and cannot be pulled high/low.
NoConn ~ 6150 3850
NoConn ~ 6250 3850
NoConn ~ 6350 3850
NoConn ~ 6450 3850
NoConn ~ 6550 3850
NoConn ~ 6650 3850
$Comp
L NCP1117ST33T3G U2
U 1 1 58C5AD2C
P 1900 950
F 0 "U2" H 1900 1217 50  0000 C CNN
F 1 "NCP1117ST33T3G" H 1900 1126 50  0000 C CNN
F 2 "TO_SOT_Packages_SMD:SOT-223-3Lead_TabPin2" H 2250 -200 50  0001 L CNN
F 3 "" H 2200 50  50  0001 C CNN
	1    1900 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	7950 5750 8250 5750
Text Notes 6850 5250 0    60   ~ 0
Reset button for ESP32
NoConn ~ 5400 6600
Text Notes 2100 4300 0    60   ~ 0
Green LED
Text Notes 2200 3100 0    60   ~ 0
Yellow LED
Text GLabel 9950 2800 0    39   Input ~ 0
IO32
Text GLabel 10650 2700 2    39   Input ~ 0
IO33
Text GLabel 9950 2700 0    39   Input ~ 0
IO25
Text GLabel 10650 2600 2    39   Input ~ 0
IO26
Text GLabel 9950 2600 0    39   Input ~ 0
IO27
Text GLabel 10700 4200 2    39   Input ~ 0
IO14
Text GLabel 9900 4200 0    39   Input ~ 0
IO13
Text GLabel 10700 3900 2    39   Input ~ 0
IO4
Text GLabel 9900 3900 0    39   Input ~ 0
IO16
Text GLabel 10700 3800 2    39   Input ~ 0
IO17
Text GLabel 9900 3800 0    39   Input ~ 0
IO18
Text GLabel 10700 3700 2    39   Input ~ 0
IO19
Text GLabel 9900 3700 0    39   Input ~ 0
IO21
Text GLabel 10700 3600 2    39   Input ~ 0
IO22
Text GLabel 9900 3600 0    39   Input ~ 0
IO23
$Comp
L +3V3 #PWR028
U 1 1 58C6729B
P 10850 2900
F 0 "#PWR028" H 10850 2750 50  0001 C CNN
F 1 "+3V3" H 10850 3040 50  0000 C CNN
F 2 "" H 10850 2900 50  0000 C CNN
F 3 "" H 10850 2900 50  0000 C CNN
	1    10850 2900
	0    1    1    0   
$EndComp
$Comp
L +5V #PWR029
U 1 1 58C673C5
P 10700 4400
F 0 "#PWR029" H 10700 4250 50  0001 C CNN
F 1 "+5V" H 10700 4540 50  0000 C CNN
F 2 "" H 10700 4400 50  0000 C CNN
F 3 "" H 10700 4400 50  0000 C CNN
	1    10700 4400
	0    1    1    0   
$EndComp
$Comp
L GND #PWR030
U 1 1 58C6749D
P 9950 3300
F 0 "#PWR030" H 9950 3050 50  0001 C CNN
F 1 "GND" H 9950 3150 50  0000 C CNN
F 2 "" H 9950 3300 50  0000 C CNN
F 3 "" H 9950 3300 50  0000 C CNN
	1    9950 3300
	0    1    1    0   
$EndComp
$Comp
L GND #PWR031
U 1 1 58C675C7
P 9950 3400
F 0 "#PWR031" H 9950 3150 50  0001 C CNN
F 1 "GND" H 9950 3250 50  0000 C CNN
F 2 "" H 9950 3400 50  0000 C CNN
F 3 "" H 9950 3400 50  0000 C CNN
	1    9950 3400
	0    1    1    0   
$EndComp
$Comp
L GND #PWR032
U 1 1 58C67624
P 10650 3300
F 0 "#PWR032" H 10650 3050 50  0001 C CNN
F 1 "GND" H 10650 3150 50  0000 C CNN
F 2 "" H 10650 3300 50  0000 C CNN
F 3 "" H 10650 3300 50  0000 C CNN
	1    10650 3300
	0    -1   -1   0   
$EndComp
$Comp
L +3V3 #PWR033
U 1 1 58C68AEF
P 9750 2900
F 0 "#PWR033" H 9750 2750 50  0001 C CNN
F 1 "+3V3" H 9750 3040 50  0000 C CNN
F 2 "" H 9750 2900 50  0000 C CNN
F 3 "" H 9750 2900 50  0000 C CNN
	1    9750 2900
	0    -1   -1   0   
$EndComp
$Comp
L +5V #PWR034
U 1 1 58C68C4C
P 9950 4400
F 0 "#PWR034" H 9950 4250 50  0001 C CNN
F 1 "+5V" H 9950 4540 50  0000 C CNN
F 2 "" H 9950 4400 50  0000 C CNN
F 3 "" H 9950 4400 50  0000 C CNN
	1    9950 4400
	0    -1   -1   0   
$EndComp
$Comp
L +5V #PWR035
U 1 1 58C68F39
P 9950 4500
F 0 "#PWR035" H 9950 4350 50  0001 C CNN
F 1 "+5V" H 9950 4640 50  0000 C CNN
F 2 "" H 9950 4500 50  0000 C CNN
F 3 "" H 9950 4500 50  0000 C CNN
	1    9950 4500
	0    -1   -1   0   
$EndComp
$Comp
L +3V3 #PWR036
U 1 1 58C69813
P 9750 3000
F 0 "#PWR036" H 9750 2850 50  0001 C CNN
F 1 "+3V3" H 9750 3140 50  0000 C CNN
F 2 "" H 9750 3000 50  0000 C CNN
F 3 "" H 9750 3000 50  0000 C CNN
	1    9750 3000
	0    -1   -1   0   
$EndComp
$Comp
L +3V3 #PWR037
U 1 1 58C6AC0E
P 10850 3000
F 0 "#PWR037" H 10850 2850 50  0001 C CNN
F 1 "+3V3" H 10850 3140 50  0000 C CNN
F 2 "" H 10850 3000 50  0000 C CNN
F 3 "" H 10850 3000 50  0000 C CNN
	1    10850 3000
	0    1    1    0   
$EndComp
$Comp
L GND #PWR038
U 1 1 58C6B22F
P 10650 3400
F 0 "#PWR038" H 10650 3150 50  0001 C CNN
F 1 "GND" H 10650 3250 50  0000 C CNN
F 2 "" H 10650 3400 50  0000 C CNN
F 3 "" H 10650 3400 50  0000 C CNN
	1    10650 3400
	0    -1   -1   0   
$EndComp
$Comp
L +5V #PWR039
U 1 1 58C6B5CA
P 10700 4500
F 0 "#PWR039" H 10700 4350 50  0001 C CNN
F 1 "+5V" H 10700 4640 50  0000 C CNN
F 2 "" H 10700 4500 50  0000 C CNN
F 3 "" H 10700 4500 50  0000 C CNN
	1    10700 4500
	0    1    1    0   
$EndComp
$Comp
L GND #PWR040
U 1 1 58C6B95B
P 10650 3500
F 0 "#PWR040" H 10650 3250 50  0001 C CNN
F 1 "GND" H 10650 3350 50  0000 C CNN
F 2 "" H 10650 3500 50  0000 C CNN
F 3 "" H 10650 3500 50  0000 C CNN
	1    10650 3500
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR041
U 1 1 58C6B9B8
P 9950 3500
F 0 "#PWR041" H 9950 3250 50  0001 C CNN
F 1 "GND" H 9950 3350 50  0000 C CNN
F 2 "" H 9950 3500 50  0000 C CNN
F 3 "" H 9950 3500 50  0000 C CNN
	1    9950 3500
	0    1    1    0   
$EndComp
Wire Wire Line
	9950 2600 10050 2600
Wire Wire Line
	10050 2700 9950 2700
Wire Wire Line
	9950 2800 10050 2800
Wire Wire Line
	10050 2900 9750 2900
Wire Wire Line
	9750 3000 10050 3000
Wire Wire Line
	9950 3300 10050 3300
Wire Wire Line
	10050 3400 9950 3400
Wire Wire Line
	9950 3500 10050 3500
Wire Wire Line
	10650 3500 10550 3500
Wire Wire Line
	10550 3400 10650 3400
Wire Wire Line
	10650 3300 10550 3300
Wire Wire Line
	10850 3000 10550 3000
Wire Wire Line
	10550 2900 10850 2900
Wire Wire Line
	10550 2700 10650 2700
Wire Wire Line
	10650 2600 10550 2600
NoConn ~ 10050 3100
NoConn ~ 10050 3200
NoConn ~ 10550 3200
NoConn ~ 10550 3100
$Comp
L ESP-WROOM-32 U4
U 1 1 58C7589C
P 6350 2700
F 0 "U4" H 6350 3687 60  0000 C CNN
F 1 "ESP-WROOM-32" H 6350 3581 60  0000 C CNN
F 2 "NorBotKiCadFootprints:ESP32-WROOM" H 0   50  60  0001 C CNN
F 3 "" H 0   50  60  0001 C CNN
	1    6350 2700
	1    0    0    -1  
$EndComp
$Comp
L SW_SPDT SW1
U 1 1 58C798EE
P 7250 5650
F 0 "SW1" H 7250 5935 50  0000 C CNN
F 1 "SW_SPDT" H 7250 5844 50  0000 C CNN
F 2 "NorBotKiCadFootprints:SlideSwitchSpringReturn" H -100 250 50  0001 C CNN
F 3 "" H -100 250 50  0001 C CNN
	1    7250 5650
	1    0    0    -1  
$EndComp
NoConn ~ 7450 5550
Wire Wire Line
	7650 5750 7450 5750
Wire Wire Line
	7400 5900 7550 5900
Wire Wire Line
	7550 5900 7550 5750
Connection ~ 7550 5750
$Comp
L PWR_FLAG #FLG042
U 1 1 58C7E2C5
P 2550 1400
F 0 "#FLG042" H -700 -225 50  0001 C CNN
F 1 "PWR_FLAG" V 2550 1528 50  0000 L CNN
F 2 "" H -700 -300 50  0001 C CNN
F 3 "" H -700 -300 50  0001 C CNN
	1    2550 1400
	0    -1   -1   0   
$EndComp
Wire Wire Line
	2550 1400 2600 1400
Connection ~ 2600 1400
NoConn ~ 5650 5150
Text GLabel 8900 4450 0    39   Input ~ 0
IO5
Text GLabel 8900 4650 0    39   Input ~ 0
IO15
NoConn ~ 8900 4450
NoConn ~ 8900 4650
Text GLabel 8850 5500 0    39   Input ~ 0
IO35
Text GLabel 7950 4450 0    39   Input ~ 0
IO34
Text GLabel 7950 4550 0    39   Input ~ 0
SENSOR_VN
Text GLabel 7950 4650 0    39   Input ~ 0
SENSOR_VP
NoConn ~ 7950 4650
NoConn ~ 7950 4550
NoConn ~ 7950 4450
Text GLabel 8850 5650 0    39   Input ~ 0
IO12
Text Notes 8400 4350 0    60   ~ 0
Strapping pins. \nUse with caution.\nDo not pull high/low.
Text Notes 7450 4250 0    60   ~ 0
Input pins only.
Text GLabel 7650 3200 2    39   Input ~ 0
IO0
Wire Wire Line
	7650 3200 7300 3200
NoConn ~ 7150 5850
$Sheet
S 9000 5100 1500 1050
U 58C9CE64
F0 "IMU" 60
F1 "IMU.sch" 60
F2 "VCC" I L 9000 5300 60 
F3 "INT" I L 9000 5500 60 
F4 "FSYNC" I L 9000 5650 60 
F5 "GND" I L 9000 5900 60 
F6 "DRDY" I R 10500 5350 60 
F7 "SCL" I R 10500 5550 60 
F8 "SDA" I R 10500 5700 60 
$EndSheet
$Comp
L CONN_02X20 J1
U 1 1 58C50AE1
P 10300 3550
F 0 "J1" H 10300 4715 50  0000 C CNN
F 1 "CONN_02X20" H 10300 4624 50  0000 C CNN
F 2 "hrs-fx8c:hrs-fx8c-40p-sv" H 5200 -2450 50  0001 C CNN
F 3 "" H 5200 -2450 50  0001 C CNN
	1    10300 3550
	1    0    0    -1  
$EndComp
Wire Wire Line
	9900 3600 10050 3600
Wire Wire Line
	10700 3600 10550 3600
Wire Wire Line
	9900 3700 10050 3700
Wire Wire Line
	10700 3700 10550 3700
Wire Wire Line
	10050 3800 9900 3800
Wire Wire Line
	10700 3800 10550 3800
Wire Wire Line
	10700 3900 10550 3900
Wire Wire Line
	10050 3900 9900 3900
$Comp
L +5V #PWR043
U 1 1 58CA173C
P 10700 4300
F 0 "#PWR043" H 10700 4150 50  0001 C CNN
F 1 "+5V" H 10700 4440 50  0000 C CNN
F 2 "" H 10700 4300 50  0000 C CNN
F 3 "" H 10700 4300 50  0000 C CNN
	1    10700 4300
	0    1    1    0   
$EndComp
$Comp
L +5V #PWR044
U 1 1 58CA1DCB
P 9950 4300
F 0 "#PWR044" H 9950 4150 50  0001 C CNN
F 1 "+5V" H 9950 4440 50  0000 C CNN
F 2 "" H 9950 4300 50  0000 C CNN
F 3 "" H 9950 4300 50  0000 C CNN
	1    9950 4300
	0    -1   -1   0   
$EndComp
Wire Wire Line
	10050 4500 9950 4500
Wire Wire Line
	9950 4400 10050 4400
Wire Wire Line
	10050 4300 9950 4300
Wire Wire Line
	10550 4500 10700 4500
Wire Wire Line
	10700 4400 10550 4400
Wire Wire Line
	10550 4300 10700 4300
Wire Wire Line
	10700 4200 10550 4200
Wire Wire Line
	9900 4200 10050 4200
NoConn ~ 10550 2800
NoConn ~ 10550 4000
NoConn ~ 10550 4100
NoConn ~ 10050 4100
NoConn ~ 10050 4000
$Comp
L +3V3 #PWR045
U 1 1 58CA8B12
P 8800 5300
F 0 "#PWR045" H 8800 5150 50  0001 C CNN
F 1 "+3V3" H 8800 5440 50  0000 C CNN
F 2 "" H 8800 5300 50  0000 C CNN
F 3 "" H 8800 5300 50  0000 C CNN
	1    8800 5300
	1    0    0    -1  
$EndComp
Wire Wire Line
	9000 5300 8800 5300
Wire Wire Line
	9000 5500 8850 5500
Text Notes 9000 6450 0    60   ~ 0
NB, GPIO_35 is Input only.\nGOIO_12 is Strapping pin.
Wire Wire Line
	9000 5650 8850 5650
$Comp
L GND #PWR046
U 1 1 58CACC62
P 8800 6050
F 0 "#PWR046" H 8800 5800 50  0001 C CNN
F 1 "GND" H 8800 5900 50  0000 C CNN
F 2 "" H 8800 6050 50  0000 C CNN
F 3 "" H 8800 6050 50  0000 C CNN
	1    8800 6050
	1    0    0    -1  
$EndComp
Wire Wire Line
	9000 5900 8800 5900
Wire Wire Line
	8800 5900 8800 6050
Text GLabel 10750 5350 2    39   Input ~ 0
IO14
$Comp
L R R9
U 1 1 58CB2BD7
P 8750 3150
F 0 "R9" H 8820 3196 50  0000 L CNN
F 1 "2k2" H 8820 3105 50  0000 L CNN
F 2 "" V 8680 3150 50  0001 C CNN
F 3 "" H 8750 3150 50  0001 C CNN
	1    8750 3150
	1    0    0    -1  
$EndComp
$Comp
L R R8
U 1 1 58CB2C6D
P 8450 3150
F 0 "R8" H 8520 3196 50  0000 L CNN
F 1 "2k2" H 8520 3105 50  0000 L CNN
F 2 "" V 8380 3150 50  0001 C CNN
F 3 "" H 8450 3150 50  0001 C CNN
	1    8450 3150
	1    0    0    -1  
$EndComp
Wire Wire Line
	8800 3450 8750 3450
Wire Wire Line
	8750 3450 8750 3300
Wire Wire Line
	8800 3550 8450 3550
Wire Wire Line
	8450 3550 8450 3300
$Comp
L +3V3 #PWR047
U 1 1 58CB3517
P 8750 2900
F 0 "#PWR047" H 8750 2750 50  0001 C CNN
F 1 "+3V3" H 8750 3040 50  0000 C CNN
F 2 "" H 8750 2900 50  0000 C CNN
F 3 "" H 8750 2900 50  0000 C CNN
	1    8750 2900
	1    0    0    -1  
$EndComp
$Comp
L +3V3 #PWR048
U 1 1 58CB35CD
P 8450 2900
F 0 "#PWR048" H 8450 2750 50  0001 C CNN
F 1 "+3V3" H 8450 3040 50  0000 C CNN
F 2 "" H 8450 2900 50  0000 C CNN
F 3 "" H 8450 2900 50  0000 C CNN
	1    8450 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	8450 2900 8450 3000
Wire Wire Line
	8750 3000 8750 2900
Text Notes 8100 2650 0    60   ~ 0
I2C pull-ups. \nOnly used when using \nboard in single-board mode.
Wire Wire Line
	10750 5350 10500 5350
Text GLabel 10750 5550 2    39   Input ~ 0
IO32
Text GLabel 10750 5700 2    39   Input ~ 0
IO33
Wire Wire Line
	10750 5550 10500 5550
Wire Wire Line
	10500 5700 10750 5700
Text GLabel 8800 3450 2    39   Input ~ 0
IO32
Text GLabel 8800 3550 2    39   Input ~ 0
IO33
Text GLabel 6850 4050 3    39   Input ~ 0
IO2
Wire Wire Line
	6850 3850 6850 4050
Text GLabel 8900 4550 0    39   Input ~ 0
IO2
NoConn ~ 8900 4550
$EndSCHEMATC
